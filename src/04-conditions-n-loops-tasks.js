...
function doRectanglesOverlap(rect1, rect2) {
  const rect1Right = rect1.top + rect1.height;
  const rect2Right = rect2.top + rect2.height;
  const rect1Len = rect1.left + rect1.width;
  const rect2Len = rect2.left + rect2.width;
  return rect1.left < rect2Len && rect1Len
    > rect2.left && rect1.top < rect2Right && rect1Right > rect2.top;
}
...
function isInsideCircle(circle, point) {
  const result = ((point.x - circle.center.x) ** 2) + ((point.y - circle.center.y) ** 2)
    < (circle.radius) ** 2;
  return result;
}
